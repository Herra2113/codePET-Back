...
export const filtrarPublicaciones = async (req, res) => {
  try {
    const publicaciones = await Publicacion.find({
      titulo: new RegExp(req.body.titulo || "", "i"),
    });
    if (publicaciones) {
      res.status(200).json(publicaciones);
    } else {
...
export const activarPublicacion = async (req, res) => {
  try {
    const publicacion = await Publicacion.findByIdAndUpdate(req.params.id, {
    ...
    if (publicacion) {
      if (req.body.active) {
        res
          .status(200)
          .json({ mensaje: "publicación activada, visible!", activa: true });
      } else {
        res
          .status(200)
          .json({ mensaje: "publicación desactivada, oculta!", activa: false });
      }
    } else {
      res.status(404).json({ mensaje: "no se encontró nada" });
    }
