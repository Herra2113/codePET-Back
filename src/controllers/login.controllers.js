    ...
    if (usuario) {
      if (compareSync(req.body.password, usuario.password)) {
        ...
        if (token) {
          res
            .status(200)
            .json({ mensaje: "contraseña ok", token, usuario, error: false });
        } else {
          res
            .status(500)
            .json({ mensaje: "error al generer token", error: true });
        }
      } else {
...
export const checkTokenAdmin = async (req, res) => {
  try {
    if (adminTokenStatus(req.headers["x-token"], process.env.SECRET_KEY)) {
      ...
      res.status(401).json({
        mensaje: "token invalido / error al decodificar token",
        error: true,
      });
    }
  } catch (err) {
    res
      .status(500)
      .json({ mensaje: "error interno / token invalido: " + err, error: true });
  }
};
